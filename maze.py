...
class Maze:
    ...
    def dfs(self) -> List:
        ...
        Returns the solution path a list of vertices using backtracking
        O(V+E) time"""
        return self.dfs_helper(self.start, set(), [])

    def dfs_helper(self, cur, visited, path) -> List:
        """DFS recursion helper
        Param: current vertex, visited set, path list"""
        visited.add(cur)
        path.append(cur)
        ...
        for vert in self.adjacencyList[cur].keys():
            if vert not in visited:
                res = self.dfs_helper(vert, visited, path)
    ...
    def _backtrace(self, table: dict) -> List:
        ...
        O(n) time where n is the number of items in the table"""
        front = self.end
        res = deque()
        res.append(self.end)
        ...
        while not(pq.is_empty()):
            ...
            for vert, weight in self.adjacencyList[u].items():
                if vert not in visited and dist[vert] > dist[u] + weight:
                    dist[vert] = dist[u] + weight
                    source[vert] = u
                    pq.enqueue(vert, dist[vert] + self.manhattan_dist(vert, self.end))
